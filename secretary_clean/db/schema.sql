...
-- Biometric credentials (fingerprint hashes) per user.
-- The hash is never the raw biometric — always a salted SHA-256 of the
-- device-local biometric template, so the server never stores raw prints.
CREATE TABLE clean_user_biometrics (
    id UUID PRIMARY KEY,
...
-- Server-side backup manifests.
CREATE TABLE clean_backup_manifests (
    id UUID PRIMARY KEY,
    company_id UUID NOT NULL REFERENCES clean_companies(id) ON DELETE CASCADE,
